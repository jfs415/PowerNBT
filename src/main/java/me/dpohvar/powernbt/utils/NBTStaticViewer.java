	...
	public static String getShortValue(Object base, boolean hex, boolean bin) {
		...
		switch (type) {
			...
			case BYTEARRAY -> { // bytearray
				...
				if (v.length == 0) {
					...
					if (hex) {
						...
						ArrayList<String> h = new ArrayList<>(v.length);
						for (byte b : v) {
							h.add(Integer.toBinaryString(b & 0xFF));
						}
			...
			case LIST -> { // list
				...
				if (listType == null) {
					listType = NBTType.END;
				}
				if (list.size() == 0) {
					value = PowerNBT.plugin.translate("data_emptylist");
				} else {
					value = PowerNBT.plugin.translate("data_elements", list.size()) + " " + listType.color + listType.name + ChatColor.RESET;
				}
			}
			...
			case INTARRAY -> { // intarray
				...
				if (v.length == 0) {
					...
					if (hex) {
						...
						for (int b : v) {
							h.add(Long.toHexString(b & 0xFFFFFFFFL));
						}
						value += "#" + StringUtils.join(h, ',');
					} else if (bin) {
						ArrayList<String> h = new ArrayList<String>(v.length);
						for (int b : v) {
							h.add(Long.toBinaryString(b & 0xFFFFFFFFL));
						}
						value += "b" + StringUtils.join(h, ',');
					} else {
						value += StringUtils.join(ArrayUtils.toObject(v), ',');
		...
		if (base == null) {
			...
			if (v.length == 0) {
				...
				for (int i = start; i < end; i++) {
					...
					if (hex) {
						buffer.append("#").append(Integer.toHexString(v[i] & 0xFF));
					} else if (bin) {
						buffer.append("b").append(Integer.toBinaryString(v[i] & 0xFF));
					} else {
						buffer.append(v[i] & 0xFF);
					}
				}
			...
			if (v.length == 0) {
				...
				for (int i = start; i < end; i++) {
					...
					buffer.append("\n").append(color).append("[").append(i).append("] ").append(ChatColor.RESET);
					if (hex) {
						buffer.append("#").append(Long.toHexString(v[i] & 0xFFFFFFFFL));
					} else if (bin) {
						buffer.append("b").append(Long.toBinaryString(v[i] & 0xFFFFFFFFL));
					} else {
						buffer.append(v[i]);
					}
				}
			...
			if (v.length == 0) {
				...
				for (int i = start; i < end; i++) {
					...
					buffer.append("\n").append(color).append("[").append(i).append("] ").append(ChatColor.RESET);
					if (hex) {
						buffer.append("#").append(Long.toHexString(v[i]));
					} else if (bin) {
						buffer.append("b").append(Long.toBinaryString(v[i]));
					} else {
						buffer.append(v[i]);
					}
				}
			...
			if (entries.size() == 0) {
				...
				for (int i = start; i < end; i++) {
					...
					if (!(key instanceof String currentName)) {
						continue;
					}
					...
					if (b instanceof NBTList nbtListEntry) {
						c = NBTType.fromByte(nbtListEntry.getType()).color;
					}
					String bolder = switch (t) {
						case LIST, COMPOUND -> ChatColor.BOLD.toString();
						default -> "";
					};
					buffer.append('\n').append(c).append(NBTType.getIconByValue(b)).append(' ').append(bolder).append(currentName).append(':').append(ChatColor.RESET).append(' ').append(getShortValue(b, hex, bin));
				}
			...
			if (array != null) {
				...
				if (array.length == 0) {
					...
					for (int i = start; i < end; i++) {
						...
						if (typeColor == ChatColor.MAGIC) {
							typeColor = ChatColor.WHITE;
						}
						buffer.append('\n').append(itemTypeColor).append(ChatColor.BOLD).append("[").append(i).append("] ").append(ChatColor.RESET).append(getShortValue(b, hex, bin));
					}
					value = PowerNBT.plugin.translate("data_elements", array.length) + " " + typeColor + typeName + buffer;
				}
			} else {
