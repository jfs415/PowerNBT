...
public interface IntegerSelector extends QSelector {
	...
	private static void putToFreeIndex(List<Object> list, int index, Object value) {
		...
		if (selectIndex < 0) {
			selectIndex = list.size() - selectIndex;
		}
		if (selectIndex < 0) {
			throw new IndexOutOfBoundsException(selectIndex);
		}
		...
		if (addDefaultCount > 0) {
			...
			if (list instanceof NBTList nbtList) {
				...
				if (nbtType != null) {
					defaultValue = nbtType.getDefaultValue();
				}
			}
			while (addDefaultCount-- > 0) {
