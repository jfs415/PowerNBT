...
public record KeySelector(String key) implements QSelector {
	...
	public Object set(Object current, Object value, boolean createDir) throws NBTTagNotFound {
		if (current == null && createDir) {
			current = new HashMap<>();
		}
		if (current instanceof Map<?, ?> map) {
			Object prevValue = map.get(key);
			if (map.containsKey(key) && Objects.equals(prevValue, value)) {
				return map;
			}
			Map resultMap = cloneMap(map);
	...
	private static Map<?, ?> cloneMap(Map<?, ?> map) {
		if (map instanceof NBTCompound c) {
			return c.clone();
		}
	...
	public String getSeparator(QSelector prevSelector) {
		...
		if (prevSelector instanceof StringAsJsonSelector) {
			return null;
		}
		return ".";
	}
	...
	public String toString() {
		if (key.isEmpty()) {
			return StringParser.wrapToQuotes(key);
		}
		...
		if (!ChatColor.stripColor(key).equals(key)) {
			return StringParser.wrapToQuotes(key);
		}
		return key;
	}
