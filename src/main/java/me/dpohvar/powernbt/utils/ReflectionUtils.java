...
public class ReflectionUtils {
	...
	private static Class<?> classByName(String pattern) throws ClassNotFoundException {
		if (pattern.endsWith("[]")) {
			return classByName(pattern.substring(0, pattern.length() - 2)).arrayType();
		}
	...
	public record RefClass<T>(Class<T> getRealClass) {
		...
		private static Class<?> getParamClass(Object type) {
			...
			if (type instanceof RefClass rc) {
				return rc.getRealClass();
			}
			if (type instanceof String s) {
				return getRefClass(s).getRealClass();
			}
		 ...
		 *
		 * @param types
		 * 			  parameters. can be Class, RefClass or String
		 * @return RefMethod object
		 * @throws RuntimeException
		 ...
		 *
		 * @param types
		 * 			  parameters. can be Class or RefClass or String
		 * @return RefMethod object
		 * @throws RuntimeException
		 * 			  if method not found
		 */
		...
		public RefMethod<?> findMethod(MethodCondition... condition) {
			...
			throw lastException != null ? lastException
			                            : new RuntimeException("no such method: " + Arrays.toString(condition) + " for class " + this.getRealClass.getName());
		}
		 ...
		 *
		 * @param pattern
		 * 			  possible names of method, split by ","
		 * @return RefMethod object
		 * @throws RuntimeException
		 * 			  if method not found
		 */
		...
		public final <Z> RefMethod<Z> findMethodByReturnType(RefClass<Z>... types) {
			...
			for (int i = 0; i < types.length; i++) {
				classes[i] = types[i].getRealClass;
			}
		 ...
		 *
		 * @param patterns
		 * 			  type of returned value, see {@link #getRefClass(String...)}
		 * @return RefMethod
		 * @throws RuntimeException
		 * 			  if method not found
		 */
		 ...
		 *
		 * @param pattern
		 * 			  field type, see {@link #getRefClass(String...)}
		 * @return RefField
		 * @throws RuntimeException
		 * 			  if field not found
		 */
	...
	public static class MethodCondition {
		...
		public String toString() {
			...
			if (names != null) {
				b.append("names:").append(Arrays.toString(names)).append(",");
			}
			if (prefix != null) {
				b.append("prefix:").append(prefix).append(",");
			}
			if (suffix != null) {
				b.append("suffix:").append(suffix).append(",");
			}
			if (checkForge) {
				b.append("forge:").append(forge).append(",");
			}
			if (types != null) {
				b.append("types:").append(types).append(",");
			}
			if (index != -1) {
				b.append("index:").append(index).append(",");
			}
			if (checkAbstract) {
				b.append("abstract:").append(modAbstract).append(",");
			}
			if (checkFinal) {
				b.append("final:").append(modFinal).append(",");
			}
			if (checkStatic) {
				b.append("modStatic:").append(modStatic).append(",");
			}
			if (checkPublic) {
				b.append("public:").append(modPublic).append(",");
			}
			if (checkPrivate) {
				b.append("private:").append(modPrivate).append(",");
			}
			if (checkProtected) {
				b.append("protected:").append(modProtected).append(",");
			}
			if (checkDeclared) {
				b.append("declared:").append(modDeclared).append(",");
			}
			if (returnType != null) {
				b.append("returnType:").append(returnType).append(",");
			}
		...
		public @NotNull MethodCondition withReturnType(@NotNull String pattern) {
			...
			if (refClass == null) {
				throw new RuntimeException("class not found: " + pattern);
			}
		...
		private RefMethod<?>[] findAll(Class<?> clazz) {
			...
			if (names != null) {
				...
				while (itr.hasNext()) {
					...
					if (Arrays.stream(names).noneMatch(name::equals)) {
						itr.remove();
					}
				}
			}
	...
	public static class RefMethod<Z> {
		...
		public String toString() {
			...
			return method.getDeclaringClass().getName() + " " + method.getName() + "(" + String.join(", ", paramTypes) + ")";
		}
		...
		public static @NotNull RefMethod<?> parse0(@NotNull String string) {
			...
			String methodReturnTypePattern =
						  split2.length > 1 ? split2[1] : null; // net.minecraft.nbt.NBTTagCompound | null
			...
			String methodParamsPattern = split3.length >= 2 ? split3[1].replaceAll("^ *|[:()]+| *$", "")
			...
			String[] methodParams;
			if (methodParamsPattern == null) {
				methodParams = null;
			} else if (methodParamsPattern.isEmpty()) {
				methodParams = new String[0];
			} else {
				methodParams = methodParamsPattern.split("[(), ]+");
			}
