	...
	private static TextComponent getValuesComponent(ViewerStyle style, ContainerControl control, List<?> list, int start, int end, int limitCol, int limitRow, boolean hex, boolean bin) {
		...
		for (int i = start; i < Math.min(toEnd, list.size()); i++) {
			...
			if (readonly) {
				...
				InteractiveElement keyNameElement = new InteractiveElement(style.getColorByValue(listValue), indexSelector.toString(), EventBuilder.runNbt(control.getSelector() + " " + control.getAccessQuery().add(indexSelector), false), EventBuilder.popup("select " + i), null);
				if (i != start) {
					component.addExtra("\n");
				}
			...
			if (readonly) {
				...
				InteractiveElement ValueElement = new InteractiveElement(null, shortValue, EventBuilder.suggestNbt(control.getSelector() + " " + control.getAccessQuery().add(indexSelector) + " = ", false), EventBuilder.popup("edit"), null);
