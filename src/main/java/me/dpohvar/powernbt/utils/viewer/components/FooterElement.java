...
public class FooterElement implements Element {
	...
	public FooterElement(ViewerStyle style, ContainerControl control, int colLimit, int rowLimit, int start, int end, boolean hex, boolean bin) {
		...
		// buttons
		List<InteractiveElement> buttonElements = Stream.of(ButtonUpdate.create(style, control, parentAccess), ButtonSelectJson.create(style, control), ButtonCopyToBuffer.create(style, control), ButtonPasteFromBuffer.create(style, control), ButtonCopyToClipboard.create(style, control), ButtonEdit.create(style, control), ButtonRemove.create(style, control, null)).filter(Objects::nonNull).toList();
		...
		if (size >= 0) {
			...
			if (start == 0 && end == 0) {
				end = (value instanceof String) ? colLimit * rowLimit : colLimit;
			}
