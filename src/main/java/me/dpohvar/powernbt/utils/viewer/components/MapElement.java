		...
		for (int i = start; i < Math.min(toEnd, map.size()); i++) {
			...
			if (key instanceof String mapKey) {
				...
				if (readonly) {
					...
					InteractiveElement keyNameElement = new InteractiveElement(style.getColorByValue(mapValue), selector.toString(), EventBuilder.runNbt(control.getSelector() + " " + control.getAccessQuery().add(selector), false), EventBuilder.popup("type " + mapValueType + "\nselect " + mapKey), null);
					if (i != start) {
						component.addExtra("\n");
					}
			...
			if (readonly || (!(key instanceof String mapKey))) {
				...
				KeySelector selector = new KeySelector(mapKey);
				InteractiveElement ValueElement = new InteractiveElement(null, shortValue, EventBuilder.suggestNbt(control.getSelector() + " " + control.getAccessQuery().add(selector) + " = ", false), EventBuilder.popup("edit"), null);
