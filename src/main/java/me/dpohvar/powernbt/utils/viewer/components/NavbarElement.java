...
public class NavbarElement implements Element {
	...
	public NavbarElement(ViewerStyle style, ContainerControl control) {
		...
		if (control.isReadonly()) {
			...
			InteractiveElement rootElement = new InteractiveElement(style.getColor("breadcrumbs.root"), control.getContainer().getRootContainer().toString(), EventBuilder.runNbt(selector, false), EventBuilder.popup("select " + control.getContainer().getRootContainer()), null);
			...
			for (int i = 0; i < steps.size(); i++) {
				...
				if (separator != null) {
					component.addExtra(delimiterColor + separator);
				}
				InteractiveElement interactiveElement = new InteractiveElement(style.getColorMod("breadcrumbs.steps", i), step.toString(), EventBuilder.runNbt(selector + " " + stepQuery, false), EventBuilder.popup("select " + step), null);
			...
			InteractiveElement nextSelectElement = new InteractiveElement(style.getColorMod("breadcrumbs.steps", steps.size()), "...", EventBuilder.suggestNbt(nextSelector, false), EventBuilder.popup("select next"), null);
