	...
	public void fillTabs(Caller caller, TabFormer former) throws Exception {
		...
		if (word.isEmpty()) {
			...
			if (caller.getOwner() instanceof Entity) {
				former.addIfStarts("block", "inventory", "item", "hand", "hand:");
			}
		...
		if (word.isEmpty()) {
			if (container == null) {
				...
				if (!(container instanceof NBTContainerValue)) {
					former.addIfStarts("=", "+=", "ren");
				}
				if (caller.readTag() != null) {
					former.addIfStarts("paste");
				}
			}
			if (container instanceof NBTContainerVariable) {
				former.addIfStarts("set");
			}
		...
		if (matches(word, "rename", "ren")) {
			...
			if (t.equals(w)) {
				former.addIfStarts(t);
			} else {
				former.addIfStarts('"' + StringParser.wrap("") + '"');
			}
			return;
			...
			int size;
			if (base instanceof Collection list) {
				size = list.size();
		...
		if (matches(oper, "=", "insert", "ins", "+=", "add", "<", "set", "select", "swap", "<>", ">", ">>", "<<")) {
			if (matches(oper, "set", "select") && !(container instanceof NBTContainerVariable)) {
				return;
			}
			word = former.poll();
			if (word.isEmpty()) {
			...
			String val2 = word;
			if (val2.equals("this") || val2.equals("self")) {
				container2 = container;
			}
			word = former.poll(); // query or type or command
			if (container2 == null && !future2) {
				return;
			}
			if (word.isEmpty()) {
				if (container2 == null) {
		...
		if (base != null) {
			if (base instanceof Map compound) {
				for (Object key : compound.keySet()) {
					...
					if (s.toUpperCase().contains(qu.toUpperCase())) {
						former.add(option + delimiter + selector);
					}
				}
			} else if (base instanceof Collection list) {
				for (int i = 0; i < list.size(); i++) {
