...
public abstract class NBTContainer<T> {
	...
	final public void setCustomTag(NBTQuery query, Object value) throws NBTTagNotFound, NBTTagUnexpectedType {
		if (query.isEmpty()) {
			...
			setCustomTag(query.set(getCustomTag(), value));
		}
	}
	...
	final public void removeCustomTag(NBTQuery query) throws NBTTagNotFound {
		if (query == null || query.isEmpty()) {
			...
			setCustomTag(query.remove(this.getCustomTag()));
		}
	}
	...
	public static String parseValueToSelector(Object value) {
		...
		if (value instanceof String s) {
			return "\"" + StringParser.wrap(s) + "\"";
		}
		if (value instanceof byte[] array) {
			return StringUtils.join(ArrayUtils.toObject(array), ',') + "b";
		}
		if (value instanceof int[] array) {
			return StringUtils.join(ArrayUtils.toObject(array), ',') + "i";
		}
		if (value instanceof long[] array) {
			return StringUtils.join(ArrayUtils.toObject(array), ',') + "l";
		}
		if (value instanceof NBTBox) {
