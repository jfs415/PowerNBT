	...
	public void execute() throws Exception {
		...
		if (worldParam == null) {
			...
			if (owner instanceof Entity) {
				world = ((Entity) owner).getWorld();
			} else if (owner instanceof BlockCommandSender) {
				world = ((BlockCommandSender) owner).getBlock().getWorld();
			} else {
			...
			if (world == null) {
				throw new RuntimeException(plugin.translate("error_noworld", worldParam));
			}
		}
